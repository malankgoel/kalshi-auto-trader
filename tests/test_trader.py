...
def test_btts_yes_selection_constant_names_label():
    assert model.BTTS_YES_SELECTION == "BTTS YES"


def test_btts_no_selection_constant_names_label():
...
def test_model_draw_key_names_game_field():
    assert model.MODEL_DRAW_KEY == "model_draw"


def test_model_away_win_key_names_game_field():
...
def test_away_team_key_names_game_field():
...
def test_fixture_team_keys_list_home_and_away_fields():
...
def test_fixture_selector_matches_optional_complete_team_selector():
    assert model.fixture_selector_matches("Argentina", "Algeria", "", "")
    assert model.fixture_selector_matches("Argentina", "Algeria", "argentina", "ALGERIA")
...
def test_date_key_names_game_field():
    assert model.DATE_KEY == "date"


def test_kickoff_utc_key_names_game_field():
...
def test_game_row_team_identity_reads_emitted_team_fields():
    row = {
        model.HOME_TEAM_KEY: "Argentina",
        model.AWAY_TEAM_KEY: "Algeria",
    }
    assert model.game_row_team_identity(row) == row


def test_game_row_home_team_reads_emitted_home_field():
    row = {model.HOME_TEAM_KEY: "Argentina"}
    assert model.game_row_home_team(row) == "Argentina"


def test_game_row_away_team_reads_emitted_away_field():
    row = {model.AWAY_TEAM_KEY: "Algeria"}
    assert model.game_row_away_team(row) == "Algeria"
...
def test_prediction_over_2_5_value_key_names_loaded_field():
    assert model.PRED_OVER_2_5_VALUE_KEY == "over_2_5"


def test_prediction_under_2_5_key_names_source_field():
...
def test_prediction_loaded_value_keys_list_numeric_loaded_fields():
    assert model.prediction_loaded_value_keys() == (
        model.PRED_HOME_WIN_VALUE_KEY,
        model.PRED_DRAW_VALUE_KEY,
        model.PRED_AWAY_WIN_VALUE_KEY,
        model.PRED_OVER_2_5_VALUE_KEY,
        model.PRED_UNDER_2_5_VALUE_KEY,
        model.PRED_BTTS_VALUE_KEY,
    )
...
def test_prediction_value_key_map_pairs_source_and_loaded_fields():
    assert model.prediction_value_key_map() == (
        (model.PRED_HOME_WIN_KEY, model.PRED_HOME_WIN_VALUE_KEY),
        (model.PRED_DRAW_KEY, model.PRED_DRAW_VALUE_KEY),
        (model.PRED_AWAY_WIN_KEY, model.PRED_AWAY_WIN_VALUE_KEY),
        (model.PRED_OVER_2_5_KEY, model.PRED_OVER_2_5_VALUE_KEY),
        (model.PRED_UNDER_2_5_KEY, model.PRED_UNDER_2_5_VALUE_KEY),
        (model.PRED_BTTS_KEY, model.PRED_BTTS_VALUE_KEY),
    )
...
def test_prediction_metadata_key_map_pairs_source_and_loaded_fields():
    assert model.prediction_metadata_key_map() == (
        (model.PRED_MATCH_ID_KEY, model.PRED_MATCH_ID_VALUE_KEY),
        (model.PRED_GROUP_KEY, model.PRED_GROUP_VALUE_KEY),
    )
...
def test_schedule_away_team_key_names_source_field():
...
def test_flag_bets_reads_shared_btts_price_keys():
    odds = {mm.BTTS_YES_PRICE_KEY: 55, mm.BTTS_NO_PRICE_KEY: 45}
    bets = model.flag_bets(_game(), odds)
    btts = [b for b in bets if b.line == "btts"][0]
    assert btts.side == "NO" and btts.market_price == approx(0.45)
...
def test_contracts_for_stake_helper():
...
def test_yes_ask_price_cents_prefers_dollar_quote():
    assert mm.yes_ask_price_cents({"yes_ask_dollars": "0.48", "yes_ask": 49}) == approx(48.0)


def test_last_price_fields_prefer_last_then_previous_quotes():
    assert mm.LAST_PRICE_FIELDS == (
        "last_price_dollars",
        "last_price",
...
def test_no_ask_price_cents_prefers_dollar_quote():
...
def test_winner_draw_line_names_order_line():
...
def test_btts_line_names_order_line():
...
def test_winner_draw_price_key_names_odds_row_field():
...
def test_totals_price_keys_group_total_outcomes():
    assert mm.totals_price_keys() == (
...
def test_btts_price_keys_group_btts_outcomes():
    assert mm.btts_price_keys() == (
...
def test_optional_no_odds_price_keys_list_complements():
    assert mm.optional_no_odds_price_keys() == (
        mm.UNDER_2_5_PRICE_KEY,
        mm.BTTS_NO_PRICE_KEY,
    )
...
def test_winner_series_reads_winner_line():
    assert mm.winner_series({
        "winner": ("KXWCGAME",),
        "over_under": ("KXWCTOTAL",),
    }) == ("KXWCGAME",)
...
def test_settlement_status_helper():
    assert trade_log.settlement_status(True) == "won"

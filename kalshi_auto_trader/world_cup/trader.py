...
def plan_bets(game: dict, bets: list, idx: dict, bankroll: float,
    ...
    for b in sorted(bets, key=lambda x: x.line):
        ...
        if ask is None or ask <= 0:
            plans.append(mark_plan_skipped(plan, SKIP_NO_ASK))
            continue
        ...
        if not risk.run_budget_allows(running, params["risk_cost"], settings.MAX_TOTAL_COST):
            plans.append(mark_plan_skipped(plan, run_cap_skip_reason(settings.MAX_TOTAL_COST)))
            continue
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        if not risk.balance_covers_cost(bal, total):

...
def _match_teams(market: dict, home: str, away: str, game_date: str = "") -> bool:
    ...
    if game_date and ev_date:
        date_diff = date_diff_days(game_date, ev_date)
...
def no_ask_price_cents(market: dict) -> Optional[float]:
    """Return the best available NO ask quote in cents."""
    return first_price_cents(market, *NO_ASK_PRICE_FIELDS)
...
def build_odds_row(idx: dict, home: str, away: str) -> dict:
    ...
    put(WINNER_HOME_PRICE_KEY, idx["winner"].get(winner_index_key(home)))
    put(WINNER_AWAY_PRICE_KEY, idx["winner"].get(winner_index_key(away)))
    put(WINNER_DRAW_PRICE_KEY, idx.get("draw"))
    put(OVER_2_5_PRICE_KEY, idx.get("over"))
    put(BTTS_YES_PRICE_KEY, idx.get("btts"))
    return row
...
def side_ask_cents(market: dict, side: str) -> Optional[float]:
    ...
    if side == "yes":
        v = first_price_cents(market, *YES_ASK_PRICE_FIELDS)
        if v is None:
            nb = no_bid_price_cents(market)
            v = 100.0 - nb if nb is not None else None
    else:  # no
        v = no_ask_price_cents(market)
        if v is None:

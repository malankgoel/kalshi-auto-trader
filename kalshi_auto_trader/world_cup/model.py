...
def upcoming_games(now: Optional[dt.datetime] = None) -> list[dict]:
    ...
    for (home, away), pred in preds.items():
        ...
        games.append({
            ...
            MODEL_HOME_WIN_KEY: pred[PRED_HOME_WIN_VALUE_KEY], MODEL_DRAW_KEY: pred[PRED_DRAW_VALUE_KEY],
            MODEL_AWAY_WIN_KEY: pred[PRED_AWAY_WIN_VALUE_KEY], MODEL_OVER_2_5_KEY: pred[PRED_OVER_2_5_VALUE_KEY],
            MODEL_BTTS_KEY: pred[PRED_BTTS_VALUE_KEY],
        })
...
def find_game(match_id: str = "", home: str = "", away: str = "") -> Optional[dict]:
    ...
    for (h, a), pred in preds.items():
        if match_id and str(pred[PRED_MATCH_ID_VALUE_KEY]) != str(match_id):
            continue
        ...
        return {
            MATCH_ID_KEY: pred[PRED_MATCH_ID_VALUE_KEY], DATE_KEY: schedule_row_date(sc) if sc else "",
            KICKOFF_UTC_KEY: ko, GROUP_KEY: pred[PRED_GROUP_VALUE_KEY], HOME_TEAM_KEY: h, AWAY_TEAM_KEY: a,
            MODEL_HOME_WIN_KEY: pred[PRED_HOME_WIN_VALUE_KEY], "model_draw": pred[PRED_DRAW_VALUE_KEY],
            "model_away_win": pred[PRED_AWAY_WIN_VALUE_KEY], "model_over_2_5": pred[PRED_OVER_2_5_VALUE_KEY],

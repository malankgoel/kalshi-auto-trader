"""Self-contained Kalshi auto-trader for the next World Cup game.

When run, it: picks the next upcoming fixture (from the bundled schedule +
pre-tournament model), pulls that game's CURRENT Kalshi odds, de-vigs them,
flags every >=10% mispricing exactly as the backtest does, sizes each with
half-Kelly, and places the orders. No tally files, no saved state -- it
recomputes from live odds every run.

Order type is one switch: --order-type market|limit (or KALSHI_ORDER_TYPE).

SAFETY: dry-run is the DEFAULT -- nothing is sent unless you pass --live. Even
live, per-order/per-run dollar caps and an account-balance check apply, and each
order carries a deterministic client_order_id so re-running before a game can
never double-place (Kalshi rejects the duplicate).

    python execute_trades.py                     # dry-run, next game, market
    python execute_trades.py --order-type limit  # dry-run, limit pricing
    python execute_trades.py --demo --live       # place on the demo exchange
    python execute_trades.py --live              # place for real
    python execute_trades.py --home France --away Senegal   # a specific game

Env for live/demo: KALSHI_API_KEY_ID, KALSHI_PRIVATE_KEY_PATH.
"""

from __future__ import annotations

import argparse
import math
import sys
import uuid

import config
import market_match as mm
import model
from kalshi_client import KalshiClient

_CLIENT_ID_NS = uuid.uuid5(uuid.NAMESPACE_URL, "kalshi-auto-trader")


# --------------------------------------------------------------------------- #
# pure helpers (unit-tested, no network)                                      #
# --------------------------------------------------------------------------- #
def size_order(stake_dollars: float, price_cents: float) -> int:
    """Whole contracts the stake buys at ``price_cents`` each, clamped to the
    per-order caps. 0 when the stake can't afford a single contract."""
    if stake_dollars <= 0 or not price_cents or price_cents <= 0:
        return 0
    count = int((stake_dollars * 100.0) // price_cents)
    count = min(count, config.MAX_CONTRACTS_PER_ORDER)
    count = min(count, int((config.MAX_ORDER_COST * 100.0) // price_cents))
    return max(count, 0)


def build_order_params(side: str, count: int, price_cents: float,
                       order_type: str) -> dict:
    """(side, count, ask) -> Kalshi order fields for the chosen type.
    market: buy_max_cost bounds slippage. limit: price on the bought side."""
    p = {"order_type": order_type, "yes_price": None, "no_price": None,
         "buy_max_cost": None}
    if order_type == "limit":
        lp = int(round(price_cents + config.LIMIT_BUFFER_CENTS))
        lp = max(config.MIN_PRICE_CENTS, min(config.MAX_PRICE_CENTS, lp))
        p["yes_price" if side == "yes" else "no_price"] = lp
        p["limit_price"] = lp
        p["est_cost"] = round(count * lp / 100.0, 2)
    else:
        p["buy_max_cost"] = int(math.ceil(count * (price_cents +
                                                    config.MARKET_SLIPPAGE_CENTS)))
        p["limit_price"] = None
        p["est_cost"] = round(count * price_cents / 100.0, 2)
    return p


def client_order_id(game: dict, selection: str) -> str:
    """One bet -> one order, stable across re-runs (so Kalshi dedups retries)."""
    key = f"{game['date']}:{game['home_team']} vs {game['away_team']}:{selection}"
    return str(uuid.uuid5(_CLIENT_ID_NS, key))


# --------------------------------------------------------------------------- #
# planning                                                                    #
# --------------------------------------------------------------------------- #
def plan_bets(bets: list, idx: dict, bankroll: float, order_type: str) -> list[dict]:
    plans: list[dict] = []
    running = 0.0
    for b in sorted(bets, key=lambda x: x.line):
        plan = {"selection": b.selection, "side": b.side, "line": b.line,
                "model_prob": b.model_prob, "fair_prob": b.fair_prob,
                "edge": b.edge, "skip": None}
        frac = model.staked_fraction(b.kelly_full)
        stake = round(bankroll * frac, 2)
        plan["stake"] = stake
        if stake <= 0:
            plan["skip"] = "no +EV Kelly stake"; plans.append(plan); continue

        market, buy_side = mm.resolve_order(idx, b.line, b.side, b.selection,
                                            b.selection_team)
        if not market:
            plan["skip"] = "no live market"; plans.append(plan); continue
        ask = mm.side_ask_cents(market, buy_side)
        if ask is None or ask <= 0:
            plan["skip"] = "no ask price"; plans.append(plan); continue

        count = size_order(stake, ask)
        if count < 1:
            plan["skip"] = "stake < 1 contract"; plans.append(plan); continue

        params = build_order_params(buy_side, count, ask, order_type)
        if running + params["est_cost"] > config.MAX_TOTAL_COST:
            plan["skip"] = f"run cap ${config.MAX_TOTAL_COST:.0f}"; plans.append(plan)
            continue
        running += params["est_cost"]
        plan.update(ticker=market.get("ticker", ""), buy_side=buy_side,
                    ask=round(ask, 1), count=count, **params)
        plans.append(plan)
    return plans


# --------------------------------------------------------------------------- #
# rendering + execution                                                       #
# --------------------------------------------------------------------------- #
def print_header(game: dict, odds: dict, bankroll: float, order_type: str,
                 live: bool) -> None:
    mode = "LIVE" if live else "DRY-RUN"
    print(f"\n=== {mode} · {order_type.upper()} · {game['home_team']} vs "
          f"{game['away_team']} ({game['date']}, kickoff {game['kickoff_utc']}) ===")
    print(f"Bankroll for sizing: ${bankroll:.2f}  ·  "
          f"edge >= {config.EDGE_THRESHOLD:.0%}  ·  half-Kelly (cap "
          f"{config.MAX_STAKE_FRACTION:.0%}/bet)")
    if not odds:
        print("No live Kalshi markets matched this game yet (may not be open). "
              "Try again closer to kickoff.")


def print_plan(plans: list[dict], live: bool) -> float:
    actionable = [p for p in plans if not p.get("skip")]
    total = sum(p["est_cost"] for p in actionable)
    print(f"\n{len(actionable)} order(s) to place / {len(plans)} flagged\n")
    hdr = (f"{'Selection':16} {'Mdl':>4} {'Fair':>4} {'Edge':>5} {'Ask':>5} "
           f"{'Cnt':>4} {'Price':>6} {'Cost':>7}")
    print(hdr); print("-" * len(hdr))
    for p in plans:
        if p.get("skip"):
            print(f"{p['selection'][:16]:16} {p['model_prob']*100:>3.0f}% "
                  f"{p['fair_prob']*100:>3.0f}% {p['edge']*100:>+4.0f}% "
                  f"{'':>5} {'':>4} {'-- ' + p['skip']:>14}")
            continue
        price = p.get("limit_price")
        price_s = f"{price}c" if price is not None else "mkt"
        print(f"{p['selection'][:16]:16} {p['model_prob']*100:>3.0f}% "
              f"{p['fair_prob']*100:>3.0f}% {p['edge']*100:>+4.0f}% {p['ask']:>4.0f}c "
              f"{p['count']:>4} {price_s:>6} ${p['est_cost']:>6.2f}")
    print("-" * len(hdr))
    print(f"{'TOTAL est. cost':>54}  ${total:.2f}  (run cap ${config.MAX_TOTAL_COST:.0f})")
    return total


def place_orders(client, game: dict, plans: list[dict], order_type: str) -> None:
    for p in [p for p in plans if not p.get("skip")]:
        coid = client_order_id(game, p["selection"])
        try:
            order = client.create_order(
                ticker=p["ticker"], action="buy", side=p["buy_side"],
                count=p["count"], order_type=order_type, client_order_id=coid,
                yes_price=p.get("yes_price"), no_price=p.get("no_price"),
                buy_max_cost=p.get("buy_max_cost"))
            print(f"  placed {p['selection']:16} {p['count']:>4} @ {p['ticker']} "
                  f"-> {order.get('status', 'submitted')} ({order.get('order_id','')})")
        except Exception as exc:  # noqa: BLE001
            msg = str(exc)
            if "409" in msg or "duplicate" in msg.lower():
                print(f"  skip   {p['selection']:16} already placed earlier (dedup)")
            else:
                print(f"  FAILED {p['selection']:16} {p['ticker']}: {msg[:160]}")


# --------------------------------------------------------------------------- #
# main                                                                        #
# --------------------------------------------------------------------------- #
def resolve_bankroll(client, args) -> float:
    if args.bankroll is not None:
        return args.bankroll
    if args.live and client.authenticated:
        try:
            cents = client.get_balance().get("balance", 0)
            if cents:
                return cents / 100.0
        except Exception as exc:  # noqa: BLE001
            print(f"(could not read balance: {exc}; using ${config.BANKROLL:.2f})")
    return config.BANKROLL


def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__,
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--order-type", choices=["market", "limit"],
                    default=config.ORDER_TYPE)
    ap.add_argument("--live", action="store_true",
                    help="actually submit orders (default: dry-run)")
    ap.add_argument("--demo", action="store_true",
                    help="use Kalshi's demo exchange")
    ap.add_argument("--match-id", help="trade a specific fixture by schedule id")
    ap.add_argument("--home", help="home team (with --away) to trade a specific game")
    ap.add_argument("--away", help="away team")
    ap.add_argument("--bankroll", type=float, default=None,
                    help="override Kelly bankroll (default: live balance, else config)")
    ap.add_argument("--max-total", type=float, default=None,
                    help="override per-run total spend cap (dollars)")
    args = ap.parse_args()

    if args.max_total is not None:
        config.MAX_TOTAL_COST = args.max_total

    client = KalshiClient(base_url=config.DEMO_BASE_URL if args.demo else None)
    if args.live and not client.authenticated:
        sys.exit("Live trading needs an API key. Set KALSHI_API_KEY_ID and "
                 "KALSHI_PRIVATE_KEY_PATH (see README).")

    if args.match_id or (args.home and args.away):
        game = model.find_game(args.match_id or "", args.home or "", args.away or "")
    else:
        game = model.next_game()
    if not game:
        sys.exit("No upcoming fixture with a model prediction was found.")

    idx = mm.build_market_index(client, game["home_team"], game["away_team"],
                                game["date"])
    odds = mm.build_odds_row(idx, game["home_team"], game["away_team"])
    bets = model.flag_bets(game, odds)

    bankroll = resolve_bankroll(client, args)
    print_header(game, odds, bankroll, args.order_type, args.live)
    if not bets:
        print("\nNo line clears the threshold; nothing to bet.")
        return

    plans = plan_bets(bets, idx, bankroll, args.order_type)
    total = print_plan(plans, args.live)

    if not args.live:
        print("\nDRY-RUN: no orders sent. Add --live to place (or --demo --live "
              "to rehearse on the demo exchange).")
        return

    try:
        bal = client.get_balance().get("balance", 0)
        print(f"\nAccount balance: ${bal/100:.2f}")
        if total * 100 > bal:
            sys.exit(f"Planned spend ${total:.2f} exceeds balance ${bal/100:.2f}. "
                     f"Aborting -- no orders sent.")
    except SystemExit:
        raise
    except Exception as exc:  # noqa: BLE001
        sys.exit(f"Could not read balance ({exc}). Aborting before placing.")

    place_orders(client, game, plans, args.order_type)
    print("\nDone.")


if __name__ == "__main__":
    main()
